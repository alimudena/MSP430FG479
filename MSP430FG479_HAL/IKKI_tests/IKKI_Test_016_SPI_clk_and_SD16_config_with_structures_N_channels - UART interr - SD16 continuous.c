/**
 * @test SPI, SD16 working, N CHANNELS 
 * @brief Tests that makes sure that the SPI is working properly and sends the values read from the SD16 working with structures
   created for configuring the different modules, sampling not only one channel but up to 5 consecutive channels.
 *             MSP430FG479
 *           -----------------
 *      /|\ |              XIN|---+
 *       |  |                 |   |
...
#endif
{
  ...
  if (state == 1) {
    // UCA0TXBUF =
    // SD16A_configuration.analog_input_ID[SD16A_configuration.analog_input_being_sampled];
    state = 2;
  ...
  if (state == 3) {
    if (!id_sent) {
      UCA0TXBUF =
          SD16A_configuration
              .analog_input_ID[SD16A_configuration.analog_input_being_sampled];
      id_sent = true;
    // state = 1;
    } else if (high_or_low) {
  ...
  //  UCA0TXBUF = MST_Data;                     // Send next value
    ...
    for(i=30;i>0;i--);                        // Add time between transmissions
  to
  */
}
...
#endif
{
  ...
  case 4: // SD16MEM0 IFG
    if (state == 2) {
      ...
      if (SD16A_configuration.analog_input_being_sampled ==
          SD16A_configuration.analog_input_count) {
        SD16A_configuration.analog_input_being_sampled = 0;
      }
      ...
      SD16INCTL0 &=
          ~(SD16INCH_0 | SD16INCH_1 | SD16INCH_2 | SD16INCH_3 | SD16INCH_4);
