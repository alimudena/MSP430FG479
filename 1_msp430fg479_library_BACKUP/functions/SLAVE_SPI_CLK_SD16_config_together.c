...
//************************** SPI configuration *****************************
  ...
  static const char Master_Slave = 'S';
...
#endif
{
    ...
    if (state == 1) {
        if (high_or_low) {
            ...
            //3 - disable USCI interrupt because both words have been sent, until SD16 interruption enables it again  
            IE2 &= ~(UCA0RXIE|UCA0TXIE); 
            //4 - enable SD16 interrupt
            SD16CCTL0 |= SD16IE;
...
#endif
{
  ...
  case 4:                                   // SD16MEM0 IFG
    ...
    if (state == 0){
      ...
      //Ones saved the SD16 value, SD16 disable
        SD16CCTL0 &= ~(SD16IE);

      //2 - Enable SPI interrupt
        IE2 |= UCA0RXIE;                 // Enabling USCI interrupt
